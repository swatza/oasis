...
# IRISS Path
'''
if os.path.exists('../iriss_lib'):
	sys.path.insert(0,'../iriss_lib/iriss')
	sys.path.insert(0,'../iriss_lib/payload/protobuf')
	#import the iriss libs
	try:
		import KAIST_pb2
		import IRISS_Packet
		import IRISS_PacketBuffer
		import IRISS_KAIST_Packet_Converter
	except ImportError, e:
		print "Failed to import IRISS Libs"
'''
# CONSTANTS
RECV_BUFF = 8192
...
class StreamReaderThread(threading.Thread):
    def __init__(self, mysubscriber, myQue,
                 shutdown_event):  # we need to hand it a queue we built for it in the main area;
    ...
    def run(self):
        ...
        socketIn = socket.socket(socket.AF_INET,
                                 socket.SOCK_DGRAM)  # Could put a socket for each ID; however that could get out of hand with the number of sockets?
        ...
        while not self.shutdown.is_set():
            ...
            readable, writable, exceptional = select.select([socketIn], [socketOut],
                                                            [socketIn])  # is this the right way??
            for s in readable:
                ...
                if verifyPacketType(newPkt.getDataType(), self.DataType):
                    if self.DataType == PyPacket.PacketDataType.PKT_JSON_STRING:
                        pktData = newPkt.getData()
                        datastr = str(pktData)
                        dictOut = json.loads(datastr)
                        self.MyQueue.put([dictOut, 'json_data'])
                        print 'loaded json string message to que'
                    else:
