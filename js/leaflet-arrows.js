	...
	var Arrow = L.FeatureGroup.extend({
		options: {
			...
			validator: function (data) {
				return typeof data.latlng !== 'undefined' && typeof data.distance !== 'undefined' && !isNaN(data.distance);
		...
		redraw: function () {
			...
			if (!this._data.valid || !this._data.distance || !this._data.angle) {
				...
				var invisibleBackgroundPolyline = new L.Polyline([theLine, theArrow], backgroundPathOption);
		...
		_calculateArrowArray: function (latlng) {
			...
			var firstEdge = this._calculateEndPoint(
				latlng,
